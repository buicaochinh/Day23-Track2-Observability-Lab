...
smoke: ## health-check all 7 services
	...
	@curl -fsS http://localhost:3000/api/health | grep -q '"database":\s*"ok"' && echo "  grafana:       OK" || (curl -fsS http://localhost:3000/api/health | grep -q '"database":"ok"' && echo "  grafana:       OK")
